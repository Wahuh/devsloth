...
impl Config {
    pub fn from_env() -> Result<Config> {
        ...
        const SECURE_COOKIES: &str = "SECURE_COOKIES";
        const WEB_DOMAIN: &str = "WEB_DOMAIN";
        const WEB_URL: &str = "WEB_URL";

        const MESSAGE: &str = "environment variable not set";
        let secure_cookies_number = env::var(SECURE_COOKIES)
            .with_context(|| format!("{} {}", SECURE_COOKIES, MESSAGE))?
            .parse::<i32>()?;
        ...
        let config = Config {
            ...
            secure_cookies: match secure_cookies_number {
                0 => false,
                1 => true,
                _ => true,
            },
